...
void rk_i2c_set_clk( int scl_rate )
{
	...
	div = 91; //DIV_ROUND_UP(i2c_rate, scl_rate * 8) - 2;
        ...
        *I2C0_CLKDIV = I2C_CLKDIV_VAL(divl, divh);
}
...
int rk_i2c_send_start_bit( void )
{
	int TimeOut = I2C_TIMEOUT_US;
        ...
        *I2C0_IPD = I2C_IPD_ALL_CLEAN;
        *I2C0_CON = I2C_CON_EN | I2C_CON_START;
        *I2C0_IEN = I2C_STARTIEN;

	while (TimeOut--) {
	    if (*I2C0_IPD & I2C_STARTIPD) {
                *I2C0_IPD = I2C_STARTIPD;
		break;
	    }
	    udelay( 1 );
	}
...
int rk_i2c_send_stop_bit( void )
{
	int TimeOut = I2C_TIMEOUT_US;
        ...
        *I2C0_IPD = I2C_IPD_ALL_CLEAN;
        *I2C0_CON = I2C_CON_EN | I2C_CON_STOP;
        *I2C0_IEN = I2C_CON_STOP;

        while (TimeOut--) {
	    if (*I2C0_IPD & I2C_STOPIPD) {
                *I2C0_IPD = I2C_STOPIPD;
	        break;
	    }
	    udelay( 1 );
	}
	    ...
	    for (i = 0; i < words_tranfered_len; i++) {
		    ...
		    if (i == 0 && j == 0) {
		        ...
		        txdata |= (*pbuf++)<<(j * 8);
                    ...
                    *(volatile unsigned int *)(I2C0_TXDATA_BASE + i) = txdata;
		}
	    }

            *I2C0_CON = I2C_CON_EN | I2C_CON_MOD(I2C_MODE_TX);
            *I2C0_MTXCNT = bytes_tranfered_len;
            *I2C0_IEN = I2C_MBTFIEN | I2C_NAKRCVIEN;

	    TimeOut = I2C_TIMEOUT_US;
	    while (TimeOut--) {
	        if (*I2C0_IPD & I2C_NAKRCVIPD) {
                    *I2C0_IPD = I2C_NAKRCVIPD;
	            err = I2C_ERROR_NOACK;
	        }
	        if (*I2C0_IPD & I2C_MBTFIPD) {
                    *I2C0_IPD = I2C_MBTFIPD;
	            break;
		}
		udelay( 1 );
	    }
...
void i2c_lcd_test( void )
{
    ...
    *GPIO2D = (0xf << 16) | 0x5;   // select I2C0
    rk_i2c_set_clk( 100000 );
    textCommand( 0x01 );           // clear display
    udelay( 50000 );
    textCommand( 0x08 | 0x04 );    // display on, no cursor
    textCommand( 0x28 );           // 2 lines
