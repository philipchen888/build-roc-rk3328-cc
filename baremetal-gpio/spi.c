...
{
    *GPIO3AL = (0x1ff << 16) | 0x124;
    *GPIO3BL = (0x7 << 16) | 0x3;
    *GPIO3AH = (0x7 << 16);
    *GPIO3_SWPORTA_DDR |= 0x20;                  // GPIO3A5 OUTPUT
    *GPIO3_SWPORTA_DR |= 0x20;                   // DC = 1
}
...
void spi_write( unsigned char *dout, int len )
{
    ...
    *SPI_ENR = 0x1;
    ...
    towrite = len;
    while (towrite) {
	if (towrite && !(*SPI_SR & SR_TF_FULL)) {
	    *SPI_TXDR = out ? *out++ : 0;
	    towrite--;
	}
    }
    while (*SPI_SR & SR_BUSY) {
        udelay( 1 );
    }
...
void rk_spi_init( int speed )
{
    int src_clk_div;

    //freq = 19800000;
    src_clk_div = 29; //594000000 / freq - 1;

    *SPI_BAUDR = 0x2;
    *CRU_CLKSEL_CON24 = ((SPI2_PLL_MASK | SPI2_DIV_MASK) << 16) | (SPI2_PLL_SELECT_GENERAL << SPI2_PLL_SHIFT | src_clk_div << SPI2_DIV_SHIFT);
