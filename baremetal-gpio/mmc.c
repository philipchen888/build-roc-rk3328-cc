...
void rk3328_mmc_set_clk( int freq )
{
    int src_clk_div;

    if ( freq > 400000 ) {
        src_clk_div = 7;               // 576 / 6 = 96
        *CRU_CLKSEL_CON30 = (0x3ff << 16) | (1 << 8) | (src_clk_div - 1);
    } else {
        src_clk_div = 31;              // 24000 / 30 = 800
        *CRU_CLKSEL_CON30 = (0x3ff << 16) | (2 << 8) | (src_clk_div - 1); 
    }
}

int dwmci_setup_bus( int freq )
{
    rk3328_mmc_set_clk( freq );
    *DWMCI_CLKENA = 0;
    *DWMCI_CLKSRC = 0;
    ...
    *DWMCI_CLKENA = DWMCI_CLKEN_ENABLE | DWMCI_CLKEN_LOW_PWR;
    *DWMCI_CMD = DWMCI_CMD_PRV_DAT_WAIT | DWMCI_CMD_UPD_CLK | DWMCI_CMD_START;
    while( *DWMCI_CMD & DWMCI_CMD_START );
    return 0;
}
